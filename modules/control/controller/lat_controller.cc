...
Status LatController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  */
  ...
  matrix_a_(2, 3) = 1.0;
  matrix_a_(3, 2) = (cf_*lf_-cr_*lr_)/iz;

  matrix_a_coeff_ = Matrix::Zero(matrix_size, matrix_size);
  matrix_a_coeff_(1, 1) = -(cf_+cr_)/mass_;
  matrix_a_coeff_(1, 3) = -cf_*lf_+cr*lr_/mass_;
  matrix_a_coeff_(3, 1) = -(cf_*lf_-cr*lr_)/iz_;
  matrix_a_coeff_(3, 3) = -(cf_*lf_*lf_+cr*lr_*lr_)/iz_;
  ...
  if (injector_->vehicle_state()->gear() == canbus::Chassis::GEAR_REVERSE) {
    ...
    steer_angle_feedforwardterm =
        (ref_curvature*wheelbase_ + kv*v*v*ref_curvature -
         matrix_k_(0,2) *
             (lr_/2/cf_ -
              lf_*v*v*mass_*ref_curvature/2/cr_/wheelbase_)) *
